...
const GoogleMapsExplore = ({ providers, userLocation, onProviderClick }) => {
  ...
  // Add provider markers using Google Geocoding
  useEffect(() => {
    const addProviderMarkers = async () => {
              ...
              content: `
                <div style="padding: 12px; max-width: 300px;">
                  ...
                  <div style="display: flex; gap: 8px; margin-top: 12px;">
                    <button onclick="window.viewProviderProfile('${provider._id}')" style="flex: 1; padding: 6px 12px; font-size: 12px; border-radius: 6px; border: none; background-color: #1976d2; color: white; cursor: pointer;">
                      View Profile
                    </button>
    ...
    if (map && providers.length > 0) {
      const timeoutId = setTimeout(addProviderMarkers, 300);
...
export default GoogleMapsExplore;