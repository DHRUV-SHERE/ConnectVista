...
const SimpleMapServices = ({ services, userLocation, onServiceClick, setViewMode }) => {
  ...
  useEffect(() => {
    const initMap = async () => {
      if (!mapRef.current || allProviders.length === 0) return;
            ...
            const popupContent = `
              <div style="min-width: 200px;">
                ...
                <div style="display: flex; gap: 8px; margin-top: 12px;">
                  <button onclick="window.viewProviderFromMap('${provider._id}')" style="flex: 1; padding: 6px 12px; font-size: 12px; border-radius: 4px; border: none; background-color: #1976d2; color: white; cursor: pointer;">View Profile</button>
                  <button onclick="window.openDirectionsFromMap(${lat}, ${lng})" style="padding: 6px 12px; font-size: 12px; border-radius: 4px; border: 1px solid #1976d2; background-color: white; color: #1976d2; cursor: pointer;">📍</button>
                </div>
              </div>
            `;
  ...
  useEffect(() => {
    window.viewProviderFromMap = (providerId) => {
    ...
    window.openDirectionsFromMap = (lat, lng) => {
      const url = `https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}`;
      window.open(url, '_blank');
    };

    return () => {
      delete window.viewProviderFromMap;
      delete window.openDirectionsFromMap;
    };
...
export default SimpleMapServices;